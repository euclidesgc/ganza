...
while read -r id; do
  ...
  for criterion in 'Planejado originalmente' 'Por que não foi possível prosseguir' 'Alternativas consideradas' 'Decisão tomada' 'Resumo da resolução' 'Reconciliação documental'; do
    grep -q "$criterion" "$feature/changes.md" || error "$id: changes.md sem '$criterion'"
  done

  while IFS=: read -r start _; do
    next="$(awk -v start="$start" 'NR > start && /^### / { print NR; exit }' "$feature/changes.md")"
    if [ -n "$next" ]; then
      block="$(sed -n "${start},$((next - 1))p" "$feature/changes.md")"
    else
      block="$(sed -n "${start},\$p" "$feature/changes.md")"
    fi
    for criterion in 'Planejado originalmente' 'Por que não foi possível prosseguir' 'Alternativas consideradas' 'Decisão tomada' 'Resumo da resolução' 'Reconciliação documental'; do
      grep -q "$criterion" <<<"$block" || error "$id: mudança em changes.md sem '$criterion'"
    done
  done < <(grep -nE '^### CHG-[0-9]{3} ' "$feature/changes.md" || true)
