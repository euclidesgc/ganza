...
O contrato é [`01_prd.md`](01_prd.md), a especificação é [`02_specs.md`](02_specs.md), as decisões são [`decisions.md`](decisions.md), e desvios futuros entram em [`changes.md`](changes.md). Estado: **planejado** · branch `feature/007-agenda-google-calendar` · **próximo passo: auditar T1.1**.
...
- [ ] **T1.1** — Criar `supabase/migrations/0017_criar_vinculo_google_calendar.sql` com vínculo, state OAuth, Vault, consumo/purge físico de PKCE, fronteira JWT/RLS e tipos Calendar em propostas. · camada **migration** · `especialista-backend`
  - `rtk run 'set -eu; rg -n -P "(?s)create table public\\.google_calendar_connections\\s*\\(.*?\\buser_id\\s+uuid\\s+not null.*?\\);" /home/euclidesgc/development/ganza/supabase/migrations/0017_criar_vinculo_google_calendar.sql; rg -n -P "(?s)create table public\\.google_oauth_authorizations\\s*\\(.*?\\buser_id\\s+uuid\\s+not null.*?\\);" /home/euclidesgc/development/ganza/supabase/migrations/0017_criar_vinculo_google_calendar.sql'` exige, separadamente, os dois DDLs nomeados, cada qual com `user_id uuid not null`.
  - `rtk run 'set -eu; rg -n -P "(?ms)^alter table public\\.google_calendar_connections enable row level security;[[:space:]]*$" /home/euclidesgc/development/ganza/supabase/migrations/0017_criar_vinculo_google_calendar.sql; rg -n -P "(?ms)^alter table public\\.google_oauth_authorizations enable row level security;[[:space:]]*$" /home/euclidesgc/development/ganza/supabase/migrations/0017_criar_vinculo_google_calendar.sql; rg -n -P "(?is)create policy\\s+[^;]+\\s+on\\s+public\\.google_calendar_connections\\b[^;]*(?:using|with check)\\s*\\([^;]*\\bauth\\.uid\\(\\)[^;]*\\buser_id\\b[^;]*\\)" /home/euclidesgc/development/ganza/supabase/migrations/0017_criar_vinculo_google_calendar.sql; rg -n -P "(?is)create policy\\s+[^;]+\\s+on\\s+public\\.google_oauth_authorizations\\b[^;]*(?:using|with check)\\s*\\([^;]*\\bauth\\.uid\\(\\)[^;]*\\buser_id\\b[^;]*\\)" /home/euclidesgc/development/ganza/supabase/migrations/0017_criar_vinculo_google_calendar.sql'` exige RLS e uma policy multiline por tabela, sem alternativa que aceite só uma delas; cada policy deve referenciar `auth.uid()` e `user_id` na cláusula de acesso.
  - `rtk run 'set -eu; rg -n -P "\\brefresh_secret_ref\\b" /home/euclidesgc/development/ganza/supabase/migrations/0017_criar_vinculo_google_calendar.sql; rg -n -P "\\bpkce_secret_ref\\b" /home/euclidesgc/development/ganza/supabase/migrations/0017_criar_vinculo_google_calendar.sql; rg -n -P "\\bvault\\.secrets\\b" /home/euclidesgc/development/ganza/supabase/migrations/0017_criar_vinculo_google_calendar.sql; ! rg -n -i -P "\\b(?:refresh_token|access_token|id_token|pkce_verifier|code_verifier)\\b\\s+(?:text|varchar(?:\\s*\\(\\s*\\d+\\s*\\))?|character varying|jsonb|bytea)\\b" /home/euclidesgc/development/ganza/supabase/migrations/0017_criar_vinculo_google_calendar.sql; ! rg -n -P "\\bconsumed_at\\b" /home/euclidesgc/development/ganza/supabase/migrations/0017_criar_vinculo_google_calendar.sql'` exige separadamente ambas as referências e o uso de `vault.secrets`, e falha se token/verificador PKCE for coluna de conteúdo em claro ou se marca de consumo substituir a remoção física.
  - Teste em banco descartável prova que consumir state válido remove a linha e o segredo PKCE, que purge de state vencido faz a mesma limpeza e que iniciar OAuth executa purge antes de inserir novo state. A evidência de aplicação é o log verde do job `migrations` da CI, que aplica 0001--0017 em Postgres novo com `ON_ERROR_STOP`; o auditor lê esse log, sem usar comando mutante como prova de despacho.
  - Teste de autorização prova que JWT não grava/consulta relação de outro usuário, que o banco deriva `user_id` de `auth.uid()`, que não existe RPC interativa `p_user_id` e que `service_role` não tem `SELECT/INSERT/UPDATE/DELETE` direto nas tabelas Calendar. A ponte pública aceita somente state e só revela dono/verificador à execução interna.
  - `rtk run 'set -eu; rg -n -P "\\bcreate_calendar_event\\b" /home/euclidesgc/development/ganza/supabase/migrations/0017_criar_vinculo_google_calendar.sql; rg -n -P "\\breschedule_calendar_event\\b" /home/euclidesgc/development/ganza/supabase/migrations/0017_criar_vinculo_google_calendar.sql; ! rg -n -i -P "\\bcalendar_events\\b" /home/euclidesgc/development/ganza/supabase/migrations/0017_criar_vinculo_google_calendar.sql'` exige os dois novos kinds, separadamente, e a ausência de entidade de eventos.
  - `rtk run 'set -eu; rg -n -P "(?m)^  migrations:\\s*$" /home/euclidesgc/development/ganza/.github/workflows/ci.yml; rg -n -P "(?ms)^  migrations:\\s*$.*?^      - name: Subir o Postgres do Supabase\\s*$.*?^        run: \\|\\s*$" /home/euclidesgc/development/ganza/.github/workflows/ci.yml; rg -n -P "(?ms)^  migrations:\\s*$.*?^      - name: Bootstrap do schema auth \\(só CI\\)\\s*$.*?^        run: docker exec -i pg psql -U supabase_admin -d postgres -v ON_ERROR_STOP=1 -q < supabase/ci-bootstrap\\.sql\\s*$" /home/euclidesgc/development/ganza/.github/workflows/ci.yml; rg -n -P "(?ms)^  migrations:\\s*$.*?^      - name: Aplicar migrations em ordem\\s*$.*?^        run: \\|\\s*$.*?^          files=\\(supabase/migrations/\\*\\.sql\\)\\s*$.*?^          for f in \\\"\\$\\{files\\[@\\]\\}\\\"; do\\s*$.*?^            docker exec -i pg psql -U supabase_admin -d postgres -v ON_ERROR_STOP=1 -q < \\\"\\$f\\\"\\s*$" /home/euclidesgc/development/ganza/.github/workflows/ci.yml'` exige no job `migrations` o Postgres, o bootstrap com `ON_ERROR_STOP` e o loop ancorado que aplica cada migration com `ON_ERROR_STOP`; é a prova não mutante da régua de aplicação.
...
|---|---|---|---|
| 1 | PR 1 | planejada | auditor em T1.1 |
