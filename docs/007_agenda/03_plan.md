...
- [x] **T2.3** — Criar `supabase/functions/calendar/handler.ts`, `index.ts`, `handler_test.ts` e allowlist em `supabase/functions/main/env.ts`/`index.ts`. · camada **backend** · `especialista-backend` · **Veredito: `CUMPRIDO`** em 2026-08-29; o supervisor refez as duas falsificações e ainda plantou erro de tipo para provar que o `-q` do `deno check` não cega o gate do CI.
