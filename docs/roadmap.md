...
- [-] **F0.8 — Backend NestJS: esqueleto.** Projeto, env validado no boot, `/health` que consulta o banco, Dockerfile multi-stage sem root, 7 testes. Aplicação criada no Coolify com domínio, healthcheck e `watch_paths`; Supabase conectado à rede predefinida. **Falta:** o merge em `develop` para o primeiro build de verdade.
