...
| D7 | **O único ambiente remoto é alimentado por `develop`.** Enquanto não existir `hml` (D3), apontar o deploy para `main` deixaria o ambiente parado até a primeira release, e a Fase 0 sem onde verificar. Quando o `hml` nascer, `develop`→hml e `main`→prod, como manda o GitFlow. | 2026-08-16 | `docs/deploy/coolify.md` |
...
| D13 | **`transactions.area_id` existe (nulável, FK para `areas`), mas o formulário não pede a área — e `area_id` não entra no payload da Edge Function.** Dinheiro não se compartimenta por área (`docs/plano.md` §6.3); obrigar a escolher entre as quatro áreas padrão para registrar um almoço pede uma decisão que o produto não quer. A coluna já nasce na `0004` para o chat preenchê-la depois sem migration nova. **Dívida conhecida: FK não respeita RLS** — nada no banco impede apontar a transação para uma área alheia. Hoje não vaza (a leitura de `transactions` segue filtrada por `user_id` e o join esbarraria na RLS de `areas`) e está mitigado porque o campo fica fora do payload. Quando o chat passar a preencher `area_id`, **quem escreve valida a posse explicitamente** — o Postgres sozinho não barra. | 2026-08-16 | `docs/01-cadastro-manual/plan.md` §1 (A1) |
| D14 | **A F0.9 não exclui transação.** Correção e exclusão são a Fase 3 (F21); a limpeza das linhas do E2E é manual, por Studio/`psql`. A política `transactions_owner` é `for all` e já cobriria o `delete` — a ausência é de UI, não de banco. | 2026-08-16 | `docs/01-cadastro-manual/plan.md` §1 (A2) |
| D15 | **As fontes Fraunces e IBM Plex Sans entram na F0.9** (tarefa T3.2), não numa tarefa de tema à parte. É o que fecha honestamente a linha "algarismos tabulares" do DoD da F0.9: sem os `.ttf`, o fallback do sistema não garante figuras tabulares e a coluna de valores dança entre linhas. **Baixa a P5.** | 2026-08-16 | `docs/01-cadastro-manual/plan.md` §1 (A3) |
...
| D18 | **O volume `deno-cache` do edge-runtime precisa ser aquecido antes do restart, e o `deploy-functions.sh` ainda não faz isso.** O primeiro deploy depois que uma função passou a importar pacote npm (`@supabase/supabase-js`) pôs o `edge-runtime` em *restart loop*: `failed to bootstrap runtime: No such file or directory`. O volume só tinha o `registry.json` de cada pacote, nunca o tarball extraído, e o runtime não completa esse download a tempo de bootar — ele reinicia antes. Destravado à mão rodando `deno cache` na mesma versão embutida no runtime (2.1.4), montando o mesmo volume. **Enquanto o script não fizer isso sozinho, todo deploy que estreie uma dependência npm derruba as functions** — e o sintoma não aponta para cache nenhum. | 2026-08-16 | `scripts/deploy-functions.sh` |
| D19 | **A `service_role` é injetada no ambiente de todo worker, inclusive os que não precisam dela.** `main/index.ts` monta o `envVars` igual para qualquer função despachada; `transactions` usa só a anon key + o JWT do usuário (correto), mas a chave que fura RLS fica acessível no processo do mesmo jeito. Achado do gate do CISO na Fase 2 — **padrão pré-existente, não nasceu naquele diff**, e por isso não bloqueou o PR. Reduzir o raio é escopar `envVars` por função, no mesmo estilo do `PUBLICAS`. Vale fazer antes de a primeira função agendada (com `service_role` legítima) coexistir com as de usuário. | 2026-08-16 | `supabase/functions/main/index.ts` |
...
- [-] **F0.9 — Cadastro manual ponta a ponta.** Uma transação criada e listada pela UI, sem IA. É o que prova o encanamento inteiro: Flutter → Edge Function → Postgres → RLS → de volta na tela. Planejada em **`docs/01-cadastro-manual/`** (`specs.md` · `prd.md` · `plan.md`): cinco fases, **1 fase = 1 PR**, migration sozinha na primeira. Decisões travadas aqui: **D13**, **D14**, **D15**. Estado por fase no §8 do `plan.md`. Uma **fase extra**, fora do plano original e aprovada pelo dev, entrou entre a 1 e a 2: o caminho até produção (**D16**, **D17**), o `app/config/prod.json` que faltava e as fontes da **D15**, antecipadas da Fase 3 — sem elas nada do que a F0.9 constrói chega ao aparelho nem ao banco real.
