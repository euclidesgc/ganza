...
| D9 | **NestJS fica — com ponto de revisão marcado.** O servidor é necessário pelas chaves (Gemini, Pluggy, Google, FCM), não pelo Supabase; Edge Functions custariam outro contêiner e o deploy por volume montado. Mas **NestJS é cerimônia grande para seis endpoints** — o bug de DI do primeiro deploy foi sintoma disso. **Revisitar ao fim da Fase 3:** se o backend continuar fino, trocar por Hono é meia tarde. Se a camada de IA, a Pluggy e a conciliação o encorparem como previsto, a estrutura se pagou. | 2026-08-16 | `CLAUDE.md` |
...
| P8 | **Role dedicado no Postgres para o backend** (`ganza_api`, permissão mínima), em vez de conectar como `postgres`. | nada hoje — o backend só faz `select 1` | Vira necessário quando o backend passar a escrever (Fase 1). É uma migration pequena; o adiamento é proporcional ao risco atual, não esquecimento. |
...
- [x] **F0.8 — Backend NestJS: esqueleto.** No ar em `https://api.ganza.bmjtech.duckdns.org/health` devolvendo `{"status":"ok","database":"reachable"}`. Env validado no boot, Dockerfile multi-stage sem root, `watch_paths`, 8 testes. O deploy falhou três vezes antes, por três causas distintas — DI, rede e binding IPv4-only —, todas documentadas em `docs/deploy/coolify.md`.
