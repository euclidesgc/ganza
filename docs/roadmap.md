...
- [-] **F0.8 — Backend NestJS: esqueleto.** Projeto, env validado no boot, `/health` que consulta o banco, Dockerfile multi-stage sem root. Aplicação no Coolify com domínio, healthcheck e `watch_paths`; Supabase conectado à rede predefinida. O primeiro deploy **falhou no boot** por um erro de DI que o CI não pegava — corrigido, com teste que monta o contêiner inteiro. **Falta:** confirmar o `/health` respondendo no domínio.
