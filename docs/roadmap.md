...
| P5 | **Fontes Fraunces e IBM Plex Sans** em `app/assets/fonts/`. | a linha "algarismos tabulares" do DoD da F0.9 | Os tokens já apontam para as famílias certas; falta baixar os `.ttf` (SIL OFL, ambas) e declarar no `pubspec.yaml`. Sem elas, os algarismos tabulares não valem e a coluna de valores vai dançar. **Deixou de ser pendência do humano: a D15 a alocou na T3.2 da F0.9** (Fase 3 do `plan.md`, licença OFL, download sem interação). Sai desta tabela quando a T3.2 mergear. |
