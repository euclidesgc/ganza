...
- **Reconciliação documental:** `03_plan.md` — a linha da **TL.4** na §9, marcada
  ...
  **Completada em 29/08/2026, ao integrar a branch que trazia este registro:** as
  demais menções que davam a remoção como certa — a §3 (inventário de testes de
  aparelho), o parágrafo da Fase 2 sobre os roteiros quebrados pelo drawer, a
  linha do risco **X3** na §7, o bloco DoD da própria TL.4 (marcado como
  cancelado, sem ser apagado) e a onda 2 do lote na §9 — passam a dizer que
  `app/patrol_test/` fica. Na mesma passagem, a §8 (Progresso) foi acertada com o
  estado real: as Fases 3 a 6 e o lote de fechamento já estavam mergeados e a
  feature já constava concluída em `docs/roadmap.md`, mas as linhas de progresso
  ainda apareciam abertas.
