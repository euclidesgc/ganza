...
- [ ] **T5.6** — Substituir o placeholder de `/configuracoes/banco` pela tela real em `app/lib/modules/settings_module/presentation/bank/`: cubit com estado `sealed`, e os quatro estados de conexão visíveis. · camada **presentation** · `especialista-apresentacao`
  ...
  **DoD da tarefa**
  ...
  - A página deixa de ser o placeholder que existe hoje: `rtk proxy grep -niE 'placeholder' app/lib/modules/settings_module/presentation/bank/settings_bank_page.dart` não devolve nenhuma linha — hoje devolve duas, o `import` e o `PlaceholderBody` —, a página mantém `static Widget pageBuilder`, e o teste de cadeia de `app/test/app_router_test.dart`, que parte de `/`, abre o menu, toca em Configurações e toca em Banco, passa a asserir na tela final **um dos quatro rótulos de estado**, e não o texto do placeholder. Sem essa última exigência a cadeia já passa hoje: rota, item de menu e página placeholder existem desde a fase anterior.
