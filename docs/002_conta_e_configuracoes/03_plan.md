...
- [ ] **T5.2** `[paralela · frente B · worktree]` — Registrar o par Client ID/Secret da Pluggy como env do Coolify e da stack local, e documentar em `docs/deploy/coolify.md`. Depende da pendência **P13**. · camada **infra** · `especialista-infra`
  ...
  **DoD da tarefa**
  ...
  - Duas provas separadas, porque são coisas diferentes: que a variável **chega ao serviço** e que o **par é válido**. `docker compose -f infra/local/docker-compose.yml exec -T functions sh -c 'test -n "$PLUGGY_CLIENT_ID" && test -n "$PLUGGY_CLIENT_SECRET" && echo presentes'` imprime `presentes` — a chamada HTTP não sai de dentro do contêiner porque ele **não tem `curl`**, medido. E, da raiz do repositório, `set -a && . ./infra/local/.env && set +a && curl -s -X POST https://api.pluggy.ai/auth -H 'Content-Type: application/json' -d "{\"clientId\":\"$PLUGGY_CLIENT_ID\",\"clientSecret\":\"$PLUGGY_CLIENT_SECRET\"}" -o /dev/null -w '%{http_code}\n'` imprime `200`. Colar só o código, nunca o corpo — o corpo traz a `apiKey`. Não existe host de sandbox na Pluggy: `api.pluggy.ai` serve os dois modos e sandbox é escolha de conector, então a palavra não aponta destino nenhum.
  - Sem as variáveis, a função responde `503` de configuração ausente, e não `500` nem exceção — e isso **já é provado por teste**, sem tocar em ambiente compartilhado: `cd supabase/functions && deno test --allow-env --allow-net bank-connections/handler_test.ts --filter '503'` imprime `ok` para o caso `503 missing_configuration` de `supabase/functions/bank-connections/handler_test.ts`, e a saída vai colada em `docs/002_conta_e_configuracoes/provas/t5_2_falha_sem_a_mudanca.md` com a data. **Retirar a variável da stack para medir isso é proibido nesta tarefa:** o projeto Docker é único e compartilhado pelos três worktrees, e a medição derrubaria o ambiente de quem estiver trabalhando ao lado.
  - A seção nova de `docs/deploy/coolify.md` **acrescenta** em vez de reescrever: `git diff --numstat -- docs/deploy/coolify.md` mostra `0` na coluna de linhas removidas. Se alguma linha existente precisar mesmo mudar, o relato da tarefa nomeia qual e por quê — o que se proíbe é a alteração silenciosa que deixa falsa uma frase vizinha.
