...
- [ ] **T5.1** `[paralela · frente A · worktree]` — Criar `supabase/migrations/0009_criar_conexoes_bancarias.sql` com `public.bank_connections`, RLS e política de dono, via skill `criar-migration`. · camada **migration** · `especialista-backend`
  ...
  **DoD da tarefa**
  ...
  - O mesmo `psql` com `-tAc "select rowsecurity from pg_tables where schemaname='public' and tablename='bank_connections'"` devolve `t`, e com `-tAc "select qual, with_check from pg_policies where tablename='bank_connections'"` devolve exatamente uma linha, trazendo `(user_id = ( SELECT auth.uid() AS uid))` nas duas colunas — é assim, já normalizada pelo Postgres, que a política de dono aparece; conferir contra `select qual from pg_policies where tablename='ai_user_credentials'`, que grava a mesma expressão desde `supabase/migrations/0007_criar_credenciais_de_ia.sql`.
...
- [ ] **T5.4** `[paralela · frente C · worktree]` — Criar a camada domain de conexão bancária em `app/lib/modules/settings_module/domain/`: entidade da conexão com o status como enum fechado, contrato e um use case por operação. · camada **domain** · `especialista-dominio`
  ...
  **DoD da tarefa**
  - `app/lib/modules/settings_module/domain/entities/bank_connection.dart` declara uma classe imutável `Equatable` com instituição, status, instante da última sincronização e identificador da conexão; o campo de status é do tipo do enum declarado em `app/lib/modules/settings_module/domain/entities/bank_connection_status.dart`, e `rtk proxy grep -n 'String .*status\|status.*String' app/lib/modules/settings_module/domain/entities/bank_connection.dart` não devolve nenhuma linha.
  - `app/lib/modules/settings_module/domain/entities/bank_connection_status.dart` declara um `enum` com exatamente quatro valores e nenhum outro, um para cada estado que a coluna de status aceita no banco — pendente, conectado, erro e desconectado, nesta ordem de sentido e sem um quinto: contar os valores lendo o arquivo. A restrição correspondente do banco é criada pela migration `supabase/migrations/0009_criar_conexoes_bancarias.sql`, que **não** precisa existir nesta árvore para esta linha ser conferida.
  - `app/lib/modules/settings_module/domain/repositories/bank_connection_repository.dart` declara `abstract interface class` com exatamente três métodos — ler a conexão, iniciar a conexão e desconectar —, todos devolvendo `Future<Either<Failure, …>>`; e `app/lib/modules/settings_module/domain/usecases/` ganha exatamente três arquivos novos de conexão bancária, um por operação, cada um com uma única classe pública cujo único método público é `call()`.
  - Rodar `rtk proxy grep -rn "^import" app/lib/modules/settings_module/domain/entities/bank_connection.dart app/lib/modules/settings_module/domain/entities/bank_connection_status.dart app/lib/modules/settings_module/domain/repositories/bank_connection_repository.dart` **nomeando os três arquivos** — o comando devolve ao menos uma linha (arquivo que faltasse faria o grep sair diferente de `0`) e nenhuma das linhas devolvidas cita `package:` que não seja `package:equatable` ou `package:fpdart`. O mesmo vale rodando sobre os três arquivos novos de `usecases/`.
