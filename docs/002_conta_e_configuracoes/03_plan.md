...
- [ ] **T5.3** — Criar a Edge Function `supabase/functions/bank-connections/` com `index.ts` de borda, `handler.ts` testável e `handler_test.ts`: emitir token de conexão, persistir o item que a Pluggy devolve, ler status e desconectar. · camada **backend** · `especialista-backend`
  ...
  **DoD da tarefa**
  ...
  - Nenhum `any` atravessa a borda: `rtk proxy grep -nE ':[[:space:]]*any|as any' supabase/functions/bank-connections/handler.ts` não devolve nenhuma linha. `rtk proxy grep -n 'bank-connections' supabase/functions/deno.json` devolve os dois arquivos novos dentro da task `check`. E `cd supabase/functions && deno fmt --check && deno lint && deno task check && deno task test` termina com código de saída `0` **e** a saída do `deno task test` nomeia os casos de `bank-connections/handler_test.ts` — o código de saída sozinho não prova nada aqui: na árvore sem esta tarefa o mesmo comando já fecha em `0`, porque a task `check` não referencia o que não existe.
