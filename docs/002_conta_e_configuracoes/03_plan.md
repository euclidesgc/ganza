...
- [x] **T4.11** — Aplicar o gating nos três pontos: `redirect` e `refreshListenable` em `app/lib/app_router.dart`, `BlocSelector` no item de chat do drawer em `app/lib/core/widgets/navigation/`, e a rota `/configuracoes/ia` com o registro do DI. **Registrar junto `/chat`, com tela mínima em `app/lib/modules/chat_module/`** — sem destino registrado o `redirect` não tem para onde desviar e a prova central da tarefa não pode ser escrita (**FD-033**). · camada **infra/presentation** · `especialista-infra` · **DoD: CUMPRIDO**
  ...
  **DoD da tarefa**
  - O gate mora no router e no item de menu, nunca no corpo de página: `rtk proxy grep -c 'aiConfigured' app/lib/app_router.dart` imprime `1` ou mais, `rtk proxy grep -rl 'aiConfigured' app/lib/core/widgets/navigation/` devolve pelo menos um caminho, e `rtk proxy grep -rl 'aiConfigured' app/lib/modules/*/presentation app/lib/app_shell.dart` não devolve nenhum caminho. A varredura é da camada `presentation` porque a construção da entidade em `app/lib/modules/settings_module/data/repositories/capabilities_source_impl.dart` é a origem do dado, não um gate. Tela alcançável é tela funcional: quem barra é o router.
...
- [ ] `rtk proxy grep -c 'aiConfigured' app/lib/app_router.dart` imprime `1` ou mais, e `rtk proxy grep -rl 'aiConfigured' app/lib/core/session/ app/lib/core/widgets/navigation/` devolve os dois caminhos — a declaração do campo no domínio e o gate no item de menu. `rtk proxy grep -rl 'aiConfigured' app/lib/modules/*/presentation app/lib/app_shell.dart` não devolve nenhum caminho: o gate não mora no corpo de página nem no shell. A construção da entidade em `app/lib/modules/settings_module/data/repositories/capabilities_source_impl.dart` fica fora da varredura — é a origem do dado, não um gate, o mesmo defeito de camada que a **CHG-023** já corrigiu na T4.11 (`03_plan.md`, bloco DoD da T4.11; `changes.md`). A exigência continua a mesma: gate fora do router e do item de menu reprova.
