...
- [x] **T5.2** `[frente B]` — Registrar o par Client ID/Secret da Pluggy como env da stack local e do Coolify, e documentar em `docs/deploy/coolify.md`. · camada **infra** · `especialista-infra` · **DoD: CUMPRIDO**

  **DoD da tarefa**
  - `infra/local/docker-compose.yml` passa as duas variáveis da Pluggy ao serviço de functions e **nenhum valor real** aparece em arquivo versionado: `rtk proxy grep -n 'PLUGGY' infra/local/docker-compose.yml` mostra só nomes e interpolação de env, e `git grep -niE 'PLUGGY_CLIENT_SECRET=[A-Za-z0-9]' -- infra/ docs/ scripts/` não devolve nenhuma linha. Os dois detalhes do comando são a prova, não estilo: **`git grep`** só enxerga o que está versionado, e o segredo real mora em `infra/local/.env`, que é ignorado e **deve** conter o valor; e a classe `[A-Za-z0-9]` evita que o padrão case a própria linha de critério, como acontecia com `[^$]`.
  ...
  - Com a stack inicializada **neste worktree** por `bash scripts/local-supabase.sh up` — é ele que gera `infra/local/.runtime.env`, ignorado pelo git e ausente numa árvore recém-criada, sem o qual o `curl` abaixo sai `000` por URL vazia em vez do `503` que a prova espera — e depois de **recriar** o serviço com `docker compose -f infra/local/docker-compose.yml up -d functions`, porque o Docker injeta `environment:` só na criação do contêiner e um `restart` não alcança variável nova, a função deixa de responder configuração ausente: `set -a; . ./infra/local/.runtime.env; set +a; curl -s -o /dev/null -w '%{http_code}\n' -X POST "$SUPABASE_URL/functions/v1/bank-connections" -H "Authorization: Bearer $JWT_DONO" -H 'Content-Type: application/json' -d '{"action":"connect_token"}'` imprime `200`. **Antes** desta tarefa o mesmo comando imprime `503`, e é essa diferença que ela produz: colar os dois códigos, com a data, em `docs/002_conta_e_configuracoes/provas/t5_2_falha_sem_a_mudanca.md`. Nunca colar o corpo — ele traz a `apiKey`. Este `200` também é o que prova que o par é válido na Pluggy, sem precisar de um `curl` de autenticação em separado, que exporia o segredo no argv do processo.
  - A seção nova de `docs/deploy/coolify.md` **acrescenta** em vez de reescrever: `git diff --numstat $(git merge-base HEAD origin/develop) -- docs/deploy/coolify.md` imprime uma linha cuja segunda coluna, a de linhas removidas, é `0`. A base fixa é o que mantém a linha verificável **depois** do commit — `git diff` sem referência compara contra o próprio HEAD e sai vazio para sempre, aprovando por ausência.
...
- [x] **T5.5** — Implementar `app/lib/modules/settings_module/data/` para a conexão bancária e trocar a capacidade de banco para vir da tabela. · camada **data** · `especialista-dados` · **DoD: CUMPRIDO**
  ...
  **DoD da tarefa**
  ...
  - `app/lib/modules/settings_module/data/repositories/capabilities_source_impl.dart` passa a derivar banco conectado da tabela: `rtk proxy grep -nE 'bankConnected:[[:space:]]*false' app/lib/modules/settings_module/data/repositories/capabilities_source_impl.dart` não devolve nenhuma linha, e o comentário que explicava o valor fixo sai junto com ele — `rtk proxy grep -n 'false' app/lib/modules/settings_module/data/repositories/capabilities_source_impl.dart` não devolve nenhuma linha, nem de código nem de prosa. Hoje o primeiro grep casa a atribuição e o segundo casa também as duas linhas de comentário: os dois falham antes desta tarefa.
  - Teste em `app/test/modules/settings_module/data/repositories/capabilities_source_impl_test.dart` assere que a capacidade de banco sai **da tabela, e não do binário**: com uma conexão ativa na fonte ela é verdadeira, e sem nenhuma linha ela é falsa — os dois casos no mesmo arquivo. Fixar a capacidade em valor constante nesse `capabilities_source_impl.dart` faz um dos dois casos falhar. A prova fica **colada**, para ser conferida só lendo: `docs/002_conta_e_configuracoes/provas/t5_5_falha_sem_a_mudanca.md` traz o diff que fixa a capacidade e a saída vermelha do teste, com a data e a árvore restaurada depois. Rodar o teste com `cd app && flutter test -r compact test/modules/settings_module`.
  - `app/lib/modules/settings_module/data/models/bank_connection_model.dart` valida com `safeParse` do zard, no padrão já usado por `app/lib/modules/settings_module/data/models/profile_model.dart`, e um status fora dos quatro valores do enum vira `Failure`, nunca um valor padrão silencioso: `app/test/modules/settings_module/data/models/bank_connection_model_test.dart` passa um status desconhecido e assere o `Left`. O mesmo arquivo de provas traz o diff que faz o model cair num padrão silencioso e a saída vermelha que ele produz.
  - `app/lib/modules/settings_module/data/repositories/` continua sendo o único lugar do módulo com `try`/`catch`: `rtk proxy grep -rn 'catch (' app/lib/modules/settings_module/` devolve só linhas dessa pasta **e** ao menos uma delas está em `app/lib/modules/settings_module/data/repositories/bank_connection_repository_impl.dart`. A segunda metade não é adorno: sem ela o comando já passa hoje, com sete linhas de outras features e nenhum arquivo desta tarefa existindo.
  - `cd app && dart format --output=none --set-exit-if-changed lib/modules/settings_module` termina com código de saída `0`, e `cd app && flutter analyze lib/modules/settings_module` também — os dois comandos entram no `app/`, porque a partir da raiz do repositório esse caminho não existe.

- [x] **T5.6** — Substituir o placeholder de `/configuracoes/banco` pela tela real em `app/lib/modules/settings_module/presentation/bank/`: cubit com estado `sealed`, e os quatro estados de conexão visíveis. · camada **presentation** · `especialista-apresentacao` · **DoD: CUMPRIDO**

  **DoD da tarefa**
  - `app/lib/modules/settings_module/presentation/bank/bank_settings_cubit.dart` declara `part 'bank_settings_state.dart'` e o arquivo `app/lib/modules/settings_module/presentation/bank/bank_settings_state.dart` abre com `part of`, trazendo o estado `sealed` com um `final class` por desfecho — o mesmo par que `ai_settings_cubit.dart` e `ai_settings_state.dart` já usam no módulo. `rtk proxy grep -rn "/data/" app/lib/modules/settings_module/presentation/bank/` não devolve nenhuma linha.
  - Teste de widget em `app/test/modules/settings_module/presentation/bank/bank_settings_page_test.dart` assere que cada um dos quatro estados da conexão — **pendente, conectado, erro e desconectado** — renderiza um rótulo textual próprio e distinto, achado por `find.text`; nenhum se distingue só por cor. São exatamente esses quatro, que são os valores de `app/lib/modules/settings_module/domain/entities/bank_connection_status.dart`: **um quinto estado de conexão reprova a linha**, e em particular "integração não configurada no servidor" chega como mensagem de texto, não como estado. Rodar com `cd app && flutter test -r compact test/modules/settings_module`.
  ...
  - A página deixa de ser o placeholder que existe hoje: `rtk proxy grep -niE 'placeholder' app/lib/modules/settings_module/presentation/bank/settings_bank_page.dart` não devolve nenhuma linha — hoje devolve **uma**, a do `PlaceholderBody(title: 'Banco')` —, a página mantém `static Widget pageBuilder`, e o teste de cadeia de `app/test/app_router_test.dart`, que parte de `/`, abre o menu, toca em Configurações e toca em Banco, passa a asserir que na tela final `find.byType(PlaceholderBody)` **não encontra nada**. Sem essa última exigência a cadeia já passa hoje: rota, item de menu e página placeholder existem desde a fase anterior. A âncora é o placeholder, e não um rótulo de estado, porque os rótulos são escolhidos por esta tarefa e ninguém consegue conferi-los antes de ela existir — que os quatro apareçam é cobrado na segunda linha, no teste dela.
  - As três reversões ficam **coladas**, para serem conferidas só lendo, em `docs/002_conta_e_configuracoes/provas/t5_6_falha_sem_a_mudanca.md`, cada uma com o diff, a saída vermelha, a data e a árvore restaurada depois: trocar o rótulo textual de um estado por distinção só de cor; fazer o desconectar agir sem confirmação; e remover a entrada de Banco da home de Configurações, que tem de derrubar o teste de cadeia.
  - Cada comando com o seu próprio `cd app`: `cd app && dart format --output=none --set-exit-if-changed lib`, `cd app && flutter analyze` e `cd app && flutter test -r compact` terminam com código de saída `0` — sem o `cd`, o `analyze` roda vazio em 2 ms e sempre passa, e o `test` falha com `Test directory "test" not found`. Da raiz do worktree, `bash scripts/gates_guard.sh; echo $?` imprime `0`. E a conciliação continua fora do escopo: `rtk proxy grep -rniE 'reconcil|concilia' app/lib/modules/settings_module/` não devolve nenhuma linha — esta metade já passa hoje e vale como invariante, não como sinal de trabalho feito; o sinal está nas linhas acima.
...
- [ ] `cd app && dart format --output=none --set-exit-if-changed .`, `flutter analyze` e `flutter test -r compact` verdes; `cd supabase/functions && deno fmt --check && deno lint && deno task check && deno task test` verde; da raiz, `bash scripts/gates_guard.sh; echo $?` imprime `0`. O alvo do format é a pasta `app/` inteira, que é o que `.github/workflows/ci.yml` roda, e `--output=none` é o que faz a linha **medir** em vez de escrever.
- [ ] `rtk proxy grep -rni 'pluggy' app/lib` devolve **exatamente uma** linha, o comentário de `app/lib/core/network/dio_factory.dart` que precede esta fase, e nenhuma sob `app/lib/modules/` — quem fala com a Pluggy é a Edge Function, e isso é invariante de arquitetura, não estilo. Exigir zero linhas seria insatisfazível: o comentário existe desde antes e diz justamente que o app não chama API de terceiro.
