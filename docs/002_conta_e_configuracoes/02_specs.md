...
| ~~B4~~ | ~~Montar a chave-mestra do cofre em volume no servidor~~ **Resolvida em 21/08/2026 por medição, não por autorização:** conferiu-se no Postgres de produção, com comandos só de leitura, que a chave-mestra **já está em volume nomeado** e que recriar o contêiner do banco não a destrói (**FD-032** em [`decisions.md`](decisions.md); saídas em `docs/deploy/coolify.md`) | nada mais. O bloqueio da **primeira chave real de IA em produção** caiu junto, e nenhum outro motivo o sustenta |
...
| X4 | **A stack local diverge da de produção quanto à chave-mestra do cofre.** Em produção ela está em volume nomeado e sobrevive à recriação do contêiner — **medido em 21/08/2026**, com comandos de leitura (**FD-032**). Em `infra/local/docker-compose.yml` não está, e **ali** recriar o contêiner de fato torna todo segredo indecifrável. **A versão anterior desta linha dizia que produção estava no mesmo estado da local, e era falsa**: a afirmação veio de inferência sobre o compose local, nunca de medição | Alinhar o compose local ao desenho de produção — mudança só no repositório —, e citar sobre produção a medição, nunca o compose local. **A restrição "nenhuma chave real em produção" deixa de existir**; o que fica são as duas condições que a reabrem, listadas no risco X16 de [`03_plan.md`](03_plan.md) |
