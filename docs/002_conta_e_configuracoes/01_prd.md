...
- **A chave-mestra do cofre está em volume no servidor — isso foi medido em 21/08/2026, não deduzido.** Conferiu-se no banco de produção, com comandos só de leitura, que a chave vive dentro de um volume nomeado: recriar o contêiner do banco **não** a destrói. **A versão anterior deste item afirmava o contrário** e cobrava, como consequência de produto, que **nenhuma chave real de IA fosse salva em produção**; aquela afirmação vinha de inferência sobre o arquivo de composição da stack **local**, e **a restrição cai junto com ela — nenhum outro motivo a sustenta**. O pior caso que resta, se o volume for removido, é cada pessoa salvar a chave de novo, porque chave de IA se reobtém no provedor. A feature continua se desenvolvendo e se provando contra a stack local descartável, agora por conveniência de trabalho e não por bloqueio. Registro em [`decisions.md`](decisions.md), FD-032, e em [`changes.md`](changes.md), CHG-020.
